<template>
  <message-bubble
    ...
    :message="message"
  >
    <div class="font">{{message.msgContent.text}} <span
        v-if="sessionObj.id == message.id && leaveMessageData"
        @click="leaveMessage"
      >填写表单</span></div>
    <WriteForm
      :isVisible="isVisible"
...
export default {
  ...
  mounted() {
    // this.IsQuote();
    // console.log('历史的',this.leaveMessageData);
    // console.log('会话',this.sessionObj);
    // console.log('message',this.message);
    console.log(this.sessionObj.id , this.message.id , this.leaveMessageData==1);
  },
  computed: {
    ...mapState({
