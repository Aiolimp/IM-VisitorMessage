...
export default {
  ...
  methods: {
    //发起评价
    evaluate() {
      console.log(this.message.payload.data.sessionId,this.sessionObj.id)
      if (this.message.payload.data.sessionId == this.sessionObj.id) {
        let params = {
          orgId: this.$route.query.orgId,
          sessionId: this.sessionObj.id,
        };
        // this.modelStatus = true
        this.Request.get("/guest/session/query/appraise", params).then(
          (res) => {
            if (res.data.status) {
              console.log(
